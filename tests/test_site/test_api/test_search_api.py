...
def call_api(_params):
    return requests.get(base_url + "v1/search_events", params=_params)
...
def test_search_right_date():
    r = call_api({"date": test_event["startDate"]})
...
def test_search_start_date_but_no_end_date():
    r = call_api({"startDate": test_event["startDate"]})
    assert r.status_code == 400
    json = r.json()
...
def test_search_end_date_but_no_start_date():
    r = call_api({"endDate": test_event["startDate"]})
    assert r.status_code == 400
    json = r.json()
...
def test_search_start_date_wrong_format():
    r = call_api({"startDate": "May 5th 2012", "endDate": test_event["endDate"]})
    assert r.status_code == 400
    json = r.json()
    assert json["message"].startswith("Couldn't parse date")
...
def test_search_start_date_in_past():
    r = call_api({"startDate": "2012-01-01", "endDate": test_event["endDate"]})
    assert r.status_code == 400
    json = r.json()
    eq_(json["message"], "Can't search in the past.")
...
def test_search_end_date_wrong_format():
    r = call_api({"startDate": test_event["startDate"], "endDate": "May 5th 2012"})
    assert r.status_code == 400
    json = r.json()
    assert json["message"].startswith("Couldn't parse date")
...
def test_search_end_date_before_start_date():
    r = call_api({"startDate": test_event["endDate"], "endDate": test_event["startDate"]})
    assert r.status_code == 400
    json = r.json()
